...
async function handleAdvancedStats(interaction: any, character: Character) {
  try {
    ...
    if (
      isDeadValue.toLowerCase() !== "true" &&
      isDeadValue.toLowerCase() !== "false"
    ) {
      await modalInteraction.reply({
        content:
          "❌ La valeur pour 'Personnage mort' doit être 'true' ou 'false'.",
        flags: ["Ephemeral"],
      });
      return;
    }
    ...
    if (
      ...
      await modalInteraction.reply({
        content:
          "❌ La valeur pour 'Personnage actif' doit être 'true' ou 'false'.",
        flags: ["Ephemeral"],
      });
      return;
    }
    ...
    try {
      ...
      await interaction.followUp({
        content:
          "❌ Une erreur est survenue lors de la mise à jour des statistiques du personnage.",
        flags: ["Ephemeral"],
      });
    }
  } catch (error) {
  ...
  try {
    ...
    if (interaction.customId.startsWith("stats_")) {
      ...
      const embed = new EmbedBuilder()
        ...
        .setDescription(
          `**${updatedCharacter.name}** a été modifié avec succès.`
        )
        .addFields(
          {
    ...
    const paValue = interaction.fields.getTextInputValue("pa_input") || "";
    const hungerValue =
      interaction.fields.getTextInputValue("hunger_input") || "";
    const isDeadValue =
      interaction.fields.getTextInputValue("is_dead_input") || "";
    const canRerollValue =
      interaction.fields.getTextInputValue("can_reroll_input") || "";
    const isActiveValue =
      interaction.fields.getTextInputValue("is_active_input") || "";
