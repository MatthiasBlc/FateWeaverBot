...
export async function getAdminLogChannel(
  ...
  try {
    // Récupérer les informations de la guilde depuis la base de données
    const guild = await apiService.getGuildByDiscordId(guildId) as { adminLogChannelId?: string } | null;
 ...
 * @returns true si le message a été envoyé, false sinon
 */
export async function sendAdminLogMessage(
  guildId: string,
  client: Client,
  message: string
): Promise<boolean> {
  try {
    ...
    if (!adminLogChannel) {
      logger.debug(`No admin log channel configured for guild ${guildId}`);
      return false;
    }
    ...
    logger.error("Error sending admin log message:", { guildId, message, error });
    return false;
  }
}
